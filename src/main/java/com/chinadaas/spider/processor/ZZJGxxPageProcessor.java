 ...
 * @author 开发者真实姓名[Andy]
 */
public class ZZJGxxPageProcessor implements PageProcessor {
	...
	public void process(Page page) {
		...
		if(rs.size() >= 41) {
			dao.update("insert into s_zzjg values(?, ?, ?, ?, ?, ?, ?, ?, '学校')", rs.get(4), rs.get(8), 
		...
		try {
			...
			dao.update("insert into s_zzjg values(?, ?, ?, ?, ?, ?, ?, ?, '学校')", "", name, 
					"", "", "", "", "", "");
	...
	public static void main(String[] args) throws IOException {
		ZZJGxxPageProcessor processor = new ZZJGxxPageProcessor();
		
		List<String> names = (List<String>) processor.getDao().query(
				"select xx.name from s_jsxx xx left join s_zzjg jg on jg.name=xx.name where jg.name is null", 
		...
		Spider.create(new ZZJGxxPageProcessor()).addUrl(urls.toArray(ss)).thread(50).run();
	}
